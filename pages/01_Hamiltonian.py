...
# --- CSS Profesional NEXUS: Inyección Directa en Contenedores Nativos ---
st.markdown("""
<style>
    ...
    /* TRUCO MAESTRO CSS: Aplicar el recuadro blanco al contenedor nativo de Streamlit */
    [data-testid="stVerticalBlock"] > div:has(div.nexus-box-main),
    [data-testid="stVerticalBlock"] > div:has(div.nexus-box-side) {
    ...
    /* Bordes superiores de color para simular tus páneles premium */
    [data-testid="stVerticalBlock"] > div:has(div.nexus-box-main) {
    ...
    /* Botón RUN SIMULATION: Centrado, pequeño y corporativo */
    .stButton>button {
...
# ==========================================
# 3. COLUMNA IZQUIERDA: CONTENEDOR SEGURO
# ==========================================
with col_main:
    # Creamos un contenedor nativo de Streamlit
    with st.container():
        # Esta clase invisible le dice a nuestro CSS que pinte de blanco TODO este bloque completo
        st.markdown("<div class='nexus-box-main'></div>", unsafe_allow_html=True)
        
        # El título ahora vive de forma nativa adentro de la caja blanca
        st.markdown("<div class='card-title-inline'> Build Hamiltonian</div>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 10px; font-weight: 800; color: #6c757d; text-transform: uppercase; margin-top: -10px; margin-bottom: 20px;'>Model Configuration Matrix</p>", unsafe_allow_html=True)
        
        # Sub-columnas internas para acomodar los controles
        ...
        with c1:
            ...
            st.markdown("<p style='font-size: 12px; font-weight: 700; color: #001f3f; margin-top: 15px; margin-bottom: 8px;'>Boundary Conditions</p>", unsafe_allow_html=True)
        ...
        # Lógica de Renderizado Matemático Dinámico
        eq = r"\hat{H} = "
        if "1D" in dim_opt:
        ...
        # Caja Gris interior de la ecuación (Perfectamente metida en el bloque blanco)
        ...
        # Botón Run compacto y acomodado abajo de todo
        st.button("Run Simulation")

# ==========================================
# 4. COLUMNA DERECHA: SIDEBAR DE INSIGHTS
# ==========================================
with col_side:
    with st.container():
        # Clase identificadora para el bloque derecho
        st.markdown("<div class='nexus-box-side'></div>", unsafe_allow_html=True)
        ...
        if "1D" in dim_opt:
            st.markdown("<p style='font-size: 11px; color: #495057; line-height: 1.4; margin-bottom: 15px;'>Modeling a <b>Quantum Ring</b>. Curvature induces a geometric phase and restricts momentum to spatial angular components $p_\phi$.</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size: 11px; color: #495057; line-height: 1.4; margin-bottom: 15px;'>Modeling a <b>2D Bulk System</b>. Translation symmetry allows continuous momentum components $\mathbf{k}$ in the interface plane.</p>", unsafe_allow_html=True)
        
        # Explicación de la Física Activa
        ...
        if not rashba and not dresselhaus:
            st.markdown("<p style='font-size: 11px; color: #495057; line-height: 1.4; margin-bottom: 15px;'>Spin degeneracy is fully preserved. No spin-orbit splitting active in the band structure.</p>", unsafe_allow_html=True)
        else:
            txt = "<ul style='font-size: 11px; color: #495057; padding-left: 15px; margin-bottom: 15px; line-height: 1.4;'>"
            ...
            if dresselhaus: txt += "<li><b>Dresselhaus:</b> Spin splitting related to bulk inversion asymmetry in non-centrosymmetric crystals.</li>"
            txt += "</ul>"
            st.markdown(txt, unsafe_allow_html=True)

        # Explicación del Campo Magnético
        if zeeman:
            ...
            st.markdown("<p style='font-size: 11px; color: #495057; line-height: 1.4; margin-bottom: 0;'><b>Zeeman splitting active</b>. Time-reversal symmetry is broken, lifting Kramers degeneracy at the $\Gamma$ point.</p>", unsafe_allow_html=True)